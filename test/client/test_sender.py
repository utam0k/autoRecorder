...
class SenderTCPTest(unittest.TestCase):
...
class SenderSerialTest(unittest.TestCase):

    def setUp(self):
        self.sock = Mock(spec=serial.Serial)
        self.sock.write.return_value = None

    def tearDown(self):
        eq_(self.sock.write.call_count, 1)
        eq_(self.sock.close.call_count, 1)

    def test_send_bytes(self):
        data = 'aaaa'.encode('utf-8')
        length = '{0:010d}'.format(len(data))
        sender.send(HOST, PORT, data, sock=self.sock, proto='serial')

        send_data = self.sock.write.call_args[0][0]
        tmp = length.encode('utf-8') + data
        eq_(send_data, tmp)
